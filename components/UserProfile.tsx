...
// ============================================================
// 🔧 BOT CONFIG — শুধু এখানে বদলাও, অন্য কোথাও না
// ============================================================
const BOT_CONFIG = {
  referralBotUsername: 'Cinaflix_Streembot',  // তোমার bot username (@ ছাড়া)
  referralAppName:     'app',                  // BotFather এ যে App Name দিয়েছ
};
// ============================================================
...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  // ── Referral link — BOT_CONFIG থেকে সরাসরি ─────────────────────────────
  const referralLink = React.useMemo(() => {
    if (!userData?.referralCode) return '';
    const botName = BOT_CONFIG.referralBotUsername.trim();
    const appName = BOT_CONFIG.referralAppName.trim();
    if (!botName) return '';
    return appName
  ...
  const shareLink = () => {
    ...
    const t = [
      `🎬 *CineFlix* — বাংলাদেশের Top Streaming App!`,
      ``,
      `🍿 Movies, Web Series, K-Drama, Anime & Exclusive content`,
      `⚡ একদম Free — Fast Speed এ যেকোনো সময় দেখো!`,
      `🪙 Join করলেই পাবে Coin — টাকায় তুলো বিকাশ/নগদে!`,
      ``,
      `👇 এখনই Join করো:`,
      referralLink,
    ].join('\n');
    window.Telegram?.WebApp?.openTelegramLink?.(
      `https://t.me/share/url?url=${encodeURIComponent(referralLink)}&text=${encodeURIComponent(t)}`
    );
    haptic('light');
  };
